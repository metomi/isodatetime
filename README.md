...
`~X` is a tilde (representing either a plus or minus sign `+` or `-`, which
we should have written as `±` but can't elsewhere for (bad) unicode
reasons...) followed by a fixed, agreed number of expanded year digits. For
example, if we agree to have 2 expanded year digits, we can represent years
from -999999 to +999999 (1000000 BC to 999999 AD). Note that 1 BC is the year
0 in the proleptic Gregorian calendar used by ISO 8601.
